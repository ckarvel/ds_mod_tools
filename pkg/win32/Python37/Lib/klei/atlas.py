...
def GenerateXMLTree( texture_filename, texture_size, bboxes, offset_amount=None ):
    ...
    for filename, bbox in list(bboxes.items()):
...
def Atlas(ims, outname, max_size=2048, scale_factor=1, ignore_exceptions=False, minimize_num_textures=True, force_square=False ):
    ...
    def LocalAtlas( im, source_idx ):
        ...
        inserted = False
        for idx, fb in list(fullboxes.items()):
            atlassize = dim
            fblist = fb[1]
    ...
    for idx, fb in list(fullboxes.items()):
        ...
        while w >= 1 or h >= 1:    
            ...
            for b in fb[1]:
                ...
                if int(b_w) > 0 and int(b_h) > 0:
                    resized_b = b.im.resize( ( int(b_w), int(b_h) ), Image.ANTIALIAS )
                    b_x, b_y = b.bbox.x / divisor, b.bbox.y / divisor
                    outim[1].paste( resized_b, ( int(b_x), int(b_y) ) )
                    contained_images[ b.name ] =  b.im
        ...
        atlases[idx] = AtlasData( list(contained_images.values()), mips, bboxes )
